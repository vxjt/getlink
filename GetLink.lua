...
local nTopID = 180100 --Update this for new item IDs, https://www.wowhead.com/items?filter=151;2;180088 Setting this to an obscenely large number should future proof the addon provided you run '/gldev build' each update, unless Blizzard changes the way item info is retrieved.
local nStepSize = 50 --Database build speed.  This can be set to 500+ on beefier computers.
local nVersion = 4 --Used to check if the addon needs to rebuild the database because of a new update.
